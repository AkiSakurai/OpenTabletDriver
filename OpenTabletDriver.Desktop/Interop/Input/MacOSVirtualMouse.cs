        ...
        private void ProcessKeyStates(int prevButtonStates, int currButtonStates)
        {
            for (int i = 0; i < 5; i++)
            {
                ...
                if (currState != prevState)
                {
                    if (currState)
                    {
                        if (_pendingX.HasValue && (_clickState == 0 || _mouseMovedSinceLastDown))
                        {
                    ...
                    {
                        if (_mouseMovedSinceLastDown || _doubleCLickStopWatch.ElapsedMilliseconds > _doubleClickIntervalInMs)
                        {
                            _clickState = 0;
                        }
                    }
